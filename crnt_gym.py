...
def _solve_with_budget(net: ReactionNetwork, seconds: float = 4.0) -> Optional[dict]:
    """Wall-clock-bounded call to solve_network(). Returns None on timeout or
    on any solver failure, rather than ever letting reset() hang -- a training
    loop cannot tolerate an environment step that might not return.
    Unix-only (SIGALRM); acceptable for this environment's target use (Linux
    training infra). A subprocess-based timeout would be the portable version."""
